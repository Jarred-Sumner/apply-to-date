...
const SECTION_ORDERING = ["why", "introduction"];
...
class UpdateApplication extends React.Component {
  static async getInitialProps({ query, store, req, isServer }) {
    try {
      ...
      const dateEventId = _.get(response, "body.data.attributes.date_event_id");
      const dateEventResponse = await getDateEvent(dateEventId);
      ...
      return {
        ...
        dateEventId: _.get(dateEventResponse, "body.data.attributes.id")
      };
    } catch (exception) {
  ...
  handleUpdateApplication = async () => {
    ...
    return updateDateEventApplication({
      photos,
      id,
      socialLinks,
      sections
...
const UpdateApplicationWithStore = withRedux(
  ...
  (state, props) => {
    ...
    return {
      ...
      application: dateEventApplicationSelector(applicationId)(state)
    };
  },
