...
export function parseResourceData(response, data) {
  if (!data) {
    return data;
...
export function camelizeAttributes(object) {
  const data = _cloneDeep(object);

  _each(data.attributes, function(value, name) {
    Object.defineProperty(data, _camelCase(name), {
      value: value,
      enumerable: true
    });
  });
...
export function parseResourceDataObject(response, data) {
    ...
    if (_isArray(value.data)) {
        ...
        get: _memoize(function() {
          var related = _map(value.data, function(related) {
            var resdata = _find(response.included, function(included) {
              return (
                included.id === related.id && included.type === related.type
              );
            });
      ...
      Object.defineProperty(result, _camelCase(name), {
        get: _memoize(function() {
          var resdata = _find(response.included, function(included) {
            return (
              included.id === value.data.id && included.type === value.data.type
            );
          });
