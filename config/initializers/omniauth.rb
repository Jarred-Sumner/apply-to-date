Rails.application.config.middleware.use OmniAuth::Builder do
  ...
  provider :facebook, Rails.application.secrets[:facebook_key], Rails.application.secrets[:facebook_secret], scope: 'email,public_profile,user_photos,user_birthday,user_friends,user_location', info_fields: 'name,email,gender,interested_in,location,short_name,is_verified,timezone,birthday,age_range,photos'
  provider :instagram, Rails.application.secrets[:instagram_key], Rails.application.secrets[:instagram_secret], scope: 'basic', provider_ignores_state: true
